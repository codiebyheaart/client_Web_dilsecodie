...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM maincontent where isactive = 1 and maincategoryid =$id ");
    $stmt->execute();
...
try {
    ...
    $stmt = $conn->prepare("SELECT * FROM mainsubcategory where parentid =$subcategory");
    $stmt->execute();
  $count = 1; 
    // set the resulting array to associative
    while ($row = $stmt->fetch()) {
...
?>
                            <ul>
                                <li> <span><?php echo  $count; ?></span><a href="SingleTutPage?category=<?php echo $row["parentid"]; ?>&&subcategory=<?php echo $row["id"]; ?>"><?php echo $mainsubcategory;?></a></li>
